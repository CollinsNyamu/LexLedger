...
import type { UserProfile, Matter } from '@/lib/types'
...
export function OnboardingClient({ 
  ...
  // Step 1 state
  const [fullName, setFullName] = useState(initialProfile?.fullName || userName || '')
  const [email] = useState(initialProfile?.email || userEmail || '')
  const [lawFirm, setLawFirm] = useState(initialProfile?.lawFirm || '')
  const [hourlyRate, setHourlyRate] = useState(initialProfile?.hourlyRate?.toString() || '')
  const [practiceArea, setPracticeArea] = useState(initialProfile?.practiceArea || '')
      ...
      startTransition(async () => {
        ...
        if (result.data) {
          setMatters(prev => [...prev, {
            ...
            matterType: result.data.matter_type || '',
          }])
        }
      })
      setMatterName('')
  ...
  const handleRemoveMatter = (id: string) => {
  ...
  const handleFinishOnboarding = () => {
    startTransition(async () => {
      // Mark onboarding as complete and create demo entries
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    value={email}
                    disabled
                    className="w-full px-4 py-2.5 bg-[#0C0E14] border border-[#1E2130] rounded-lg text-[#6B7080] cursor-not-allowed"
                  />
                </div>
                <div>
              ...
              <div className="bg-[#13151E] border border-[#1E2130] rounded-xl p-5 space-y-4">
                ...
                <div className="flex justify-end">
                  <button
                    ...
                    disabled={!matterName || !clientName || !matterType || !clientEmail || isPending}
                    className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                      matterName && clientName && matterType && clientEmail && !isPending
                        ? 'bg-[#4F7EF7] text-white hover:bg-[#3D6AE0]'
                        : 'bg-[#1E2130] text-[#4A4F62] cursor-not-allowed'
                    }`}
                  >
                    {isPending ? 'Adding...' : 'Add matter'}
                  </button>
                </div>
              </div>
                ...
                <div className="space-y-3">
                  {matters.map((matter) => (
                    ...
                    >
                      ...
                      <button
                        onClick={() => handleRemoveMatter(matter.id)}
          ...
          {onboardingStep === 4 && (
            <div className="text-center space-y-8">
              ...
              <button
                ...
                disabled={isPending}
                className="w-full py-3 bg-[#4F7EF7] text-white font-medium rounded-lg hover:bg-[#3D6AE0] transition-colors disabled:opacity-50"
              >
      ...
      {onboardingStep < 4 && (
        <footer className="border-t border-[#1E2130] px-6 py-4">
          <div className="max-w-xl mx-auto flex justify-between">
            <button
              onClick={handleBack}
              disabled={onboardingStep === 1 || isPending}
